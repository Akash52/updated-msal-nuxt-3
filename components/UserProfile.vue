  ...
  >
    <div class="flex-1 flex flex-col p-8 relative">
      <div v-if="profileImage">
        <img
          class="w-32 h-32 flex-shrink-0 mx-auto rounded-full ring-4 ring-green-300"
          :src="profileImage"
          alt=""
        />
      </div>
    ...
    >
      <svg
        class="relative left-[calc(50%-11rem)] -z-10 h-[21.1875rem] max-w-none -translate-x-1/2 rotate-[30deg] sm:left-[calc(50%-30rem)] sm:h-[42.375rem] animate-pulse"
...
const getProfileImage = async () => {
  const response = await fetch(
    ...
    {
      headers: {
        Authorization: `Bearer ${user.value.bearerToken}`,
      },
    }
  );
